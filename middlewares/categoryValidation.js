...
const validateDeleteCategory = async (req, res, next) => {
  try {
    ...
    if (productCount > 0) {
      return ResponseHandler.badRequest(res, API_RESPONSES.ERROR.CATEGORY.HAS_PRODUCTS);
    }

    // Check if category has subcategories
    if (category.subcategories && category.subcategories.length > 0) {
      return ResponseHandler.badRequest(res, API_RESPONSES.ERROR.CATEGORY.HAS_SUBCATEGORIES);
    }
